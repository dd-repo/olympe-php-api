...
$a->setExecute(function() use ($a)
{
	...
	switch( $type )
	{
		case 'mysql':
			...
			$link->query("GRANT ALL PRIVILEGES ON `{$base}` . * TO '{$base}'@'%'");
			$link->query("FLUSH PRIVILEGES");
		break;
		case 'pgsql':
			$server = 'pgsql.olympe.in';
			$command = "/dns/tm/sys/usr/local/bin/create-db-pgsql {$base} {$pass} {$server}";
			$GLOBALS['gearman']->sendAsync($command);
		break;
		case 'mongodb':
			$server = 'mongo.olympe.in';
			$command = "/dns/tm/sys/usr/local/bin/create-db-mongodb {$base} {$pass} {$server}";
			$GLOBALS['gearman']->sendAsync($command);
		break;
	}
